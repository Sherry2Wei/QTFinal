...
portfilio1 = pd.DataFrame({"ret": long.ret-short.ret})

port = portfilio1_data
port["signal"] = np.where(((port.div_lag3 == port.div_lag3) & (port.divtype == "3")) |
                          ((port.div_lag6 == port.div_lag6) & (port.divtype == "4")) |
                          ((port.div_lag12 == port.div_lag12) & (port.divtype == "5")), "L",
                          np.where(((port.div_lag3 != port.div_lag3) & (port.divtype == "3")) |
                          ((port.div_lag6 != port.div_lag6) & (port.divtype == "4")) |
                          ((port.div_lag12 != port.div_lag12) & (port.divtype == "5")), "S", ""))
port["ewret"] = port.groupby(["date", "signal"])["ret"].transform("mean")
portfilio2 = pd.DataFrame({"ret": port.query('signal in(["L","S"])').groupby(
                                 ["date"]).apply(lambda x: sum(np.where(x.signal == "L",
                                                                        x.ewret, x.ewret*-1)))})

portfilio1['cumulative_ret'] = np.cumprod(portfilio1['ret']+1)
portfilio2['cumulative_ret'] = np.cumprod(portfilio2['ret']+1)
x = portfilio1.index
